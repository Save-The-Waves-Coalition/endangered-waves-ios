...
open class LightboxController: UIViewController {
    ...
    open fileprivate(set) var currentPage = 0 {
        didSet {
            ...
            configurePages(self.initialImages)
    ...
    open weak var actionDelegate: LightboxControllerActionDelegate?
...
//            pageView?.playButton.alpha = alpha
    ...
    func calculatePreloadIndicies () -> [Int] {
        ...
        if preload > 0 {
            let lbValue = max(0, currentPage - preload)
            let rbValue = min(initialImages.count, currentPage + preload)
            for iCounter in lbValue..<rbValue {
...
extension LightboxController: HeaderViewDelegate {
    ...
    func headerView(_ headerView: HeaderView, didPressDeleteButton deleteButton: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
            ...
            self.actionDelegate?.lightboxController(self, didDelete: self.images[prevIndex], at: prevIndex)
    ...
    func headerView(_ headerView: HeaderView, didPressEditButton editButton: UIButton) {
        actionDelegate?.lightboxController(self, didEdit: images[currentPage], at: currentPage)
        dismissalDelegate?.lightboxControllerWillDismiss(self)
        dismiss(animated: true, completion: nil)
    }
}
...
extension LightboxController: FooterViewDelegate {
    ...
    public func footerView(_ footerView: FooterView, didExpand expanded: Bool) {
        UIView.animate(withDuration: 0.25, animations: {
            ...
            self.headerView.editButton.alpha = expanded ? 0.0 : 1.0
        })
    }
}
